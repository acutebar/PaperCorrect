...
Requiring $\rho_i$ to be strictly $C^\infty$ is computationally unfeasible, since the derivatives are complicated; however, partitions of unity can be constructed such that $\rho_i \in C^k$ instead, ensuring the first
...
\]
It turns out that $\rho_{j, k} \in C^k(\R)$ and has support contained in $U_{j,k} = [x_j, x_{j+k+1})$. Moreover, the derivatives of $\rho_{j,k}$ are easily computed analytically. 
...
\[
  U_{jk, K} \defeq \bigcup_{a,b=0}^{i=K}P_{(j+a) (k+b)}
\]
