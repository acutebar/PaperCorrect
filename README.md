...
| **4. Energy evaluation** | The geodesic energy is assembled from the fitted curves and the current surface normal.                                                                                                                                    | `evaluate_penalties` in `detector/energy.py`                                |
| **5. Gradient descent**  | The guessed surface is discretized on a uniform grid and initialized to the flat plane. The energy is differentiated end to end by PyTorch reverse-mode autodifferentiation, and the surface is updated until convergence. | `run_vanilla_descent`, `run_adam_descent` in `detector/gradient_descent.py` |
